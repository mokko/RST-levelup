...
creates temp file in $lib (B.xml, temp.mpx). If $joinmpx is created that signals that join process is
complete.
'''
...
class XmlJoin:
    def __init__ (self, conf):
        ...
        if not os.path.isfile(conf['joinmpx']):
            if len(dirlist) > 1:
