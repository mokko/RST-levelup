<xsl:stylesheet version="2.0" xmlns="http://www.mpx.org/mpx"
		...
		<!-- schemaLocation temporarily removed -->
				...
				<xsl:if test="@mulId ne '' ">
					<xsl:element name="multimediaobjekt">
						...
						<xsl:attribute name="exportdatum">
							<xsl:for-each-group select="/museumPlusExport/multimediaobjekt[@mulId]/@exportdatum"
								group-by=".">
			...
			<xsl:for-each-group select="/museumPlusExport/personKörperschaft[@kueId]" group-by="@kueId">
				...
				<xsl:element name="personKörperschaft">
					...
					<xsl:attribute name="exportdatum">
						<xsl:for-each-group select="/museumPlusExport/personKörperschaft[@kueId]/@exportdatum"
							group-by=".">
							<xsl:sort data-type="text" order="descending"/>
							<xsl:if test="position() = 1">
								<xsl:value-of select="."/>
			...
			<xsl:for-each-group select="/museumPlusExport/sammlungsobjekt[@objId]" group-by="@objId">
				...
				<xsl:element name="sammlungsobjekt">
					...
					<xsl:attribute name="exportdatum">
						<xsl:for-each-group select="/museumPlusExport/sammlungsobjekt[@objId]/@exportdatum"
							group-by=".">
							<xsl:sort data-type="text" order="descending"/>
							<xsl:if test="position() = 1">
								<xsl:value-of select="."/>
	...
	<xsl:template name="independents">
		...
		-->
		<xsl:for-each-group select="/museumPlusExport/multimediaobjekt[@mulId=$currentId]" group-by="name($tag)">
